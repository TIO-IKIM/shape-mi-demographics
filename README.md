...
A BibTeX/CFF record is in [`CITATION.cff`](CITATION.cff). The paper PDF is not hosted here yet: publication is governed by a Springer Nature licence to publish, which sets when each version may be made public. See [`paper/`](paper/) for details, and write to the corresponding author if you need a copy in the meantime. The poster, including the poster pitch, can be found here: [`poster/`](poster/)
